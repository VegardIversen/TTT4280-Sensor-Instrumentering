...
C = np.sqrt((3*mua*(musr + mua))) #fra labhefte
d = 0.017 #tykkelse av finger i meter. 
opt_pen_depth = np.sqrt(1/(3*(musr + mua)*mua)) #optisk penetrasjonsdybde
rel_reflect = np.sqrt(3*((musr/mua) + 1)) # Relativ refleksjon.

transmitans = np.exp(-C*d)
print(transmitans)
print(rel_reflect)
print(opt_pen_depth)